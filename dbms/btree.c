...
// Minimum p is necessarily 3.  We set the maximum
// p arbitrarily.  You may change the maximum p.
#define MIN_p 3
#define MAX_p 20
 ...
 * pointers, if the vout flag is set.
 */
void print_leaves( node * root ) {
	int i;
	node * c = root;
	...
	while (true) {
		...
		if (vout)
			printf("%lx ", (unsigned long)c->pointers[p - 1]);
		if (c->pointers[p - 1] != NULL) {
			printf(" | ");
			c = c->pointers[p - 1];
		}
		else
 ...
 * line in level (rank) p, with the 
 * keys in each node and the '|' symbol
 * to separate nodes.
 * With the vout flag set.
...
void print_tree( node * root ) {
	...
	while( queue != NULL ) {
		...
		if (vout) {
			if (n->is_leaf) 
				printf("%lx ", (unsigned long)n->pointers[p - 1]);
			else
...
node * make_node( void ) {
	...
	new_node->pointers = malloc( p * sizeof(void *) );
 ...
 * the tree's p, causing the leaf to be split
 * in half.
 */
node * insert_into_leaf_after_splitting(node * root, node * leaf, int key, record * pointer) {
	...
	temp_pointers = malloc( p * sizeof(void *) );
	...
	new_leaf->pointers[p - 1] = leaf->pointers[p - 1];
	leaf->pointers[p - 1] = new_leaf;
 ...
 * the p, and causing the node to split into two.
 */
node * insert_into_node_after_splitting(node * root, node * old_node, int left_index, 
	...
	temp_pointers = malloc( (p + 1) * sizeof(node *) );
	if (temp_pointers == NULL) {
...
node * coalesce_nodes(node * root, node * n, node * neighbor, int neighbor_index, int k_prime) {
	 ...
	 * single coalesced node to exceed the limit p - 1.
	 ...
	 * Otherwise, append only cut(p) - 2 keys and
	 * cut(p) - 1 pointers.
	 */
	 ...
	 */
		...
		neighbor->pointers[p - 1] = n->pointers[p - 1];
	}
...
node * delete_entry( node * root, node * n, int key, void * pointer ) {
	...
	min_keys = n->is_leaf ? cut(p - 1) : cut(p) - 1;
